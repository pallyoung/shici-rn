...
const REFRERNCE_DATE = new Date(1949, 10, 1);
...
class Lunar {
    ...
    getLunarMonth() {
        let lunarYear = this.getLunarYear();
        //获取立春
        let lichun = Lunar.getTerm(3);
        let tiangan = '';
        switch (true) {
            case /^[甲己]/.test(lunarYear):
                tiangan = TIAN_GAN[(2 + this._month) % 10];
                break;
            case /^[乙庚]/.test(lunarYear):
                tiangan = TIAN_GAN[(4 + this._month) % 10];;
                break;
            case /^[丙辛]/.test(lunarYear):
                tiangan = TIAN_GAN[(6 + this._month) % 10];;
                break;
            case /^[丁壬]/.test(lunarYear):
                tiangan = TIAN_GAN[(8 + this._month) % 10];;
                break;
            case /^[戊癸]/.test(lunarYear):
                tiangan = TIAN_GAN[(this._month) % 10];;
                break;
        }
        let dizhi = ''
        dizhi = DI_ZHI[(this._month + 2) % 12]
    ...
    getLunarYear() {
        ...
        return TIAN_GAN[tiangan - 1] + DI_ZHI[dizhi - 1];
    }
    getDate() {
        return this._date;
    }
    ...
    toLocalString() {
        return `${this.getLunarYear()}年 ${this.getLunarMonth()}月`;
    }
    toSolarDate() {
